...
- Current canonical package (profile slice delivered): IP-2.4's profile-edit slice makes `PUT /profile` return the updated safe `/me`-shaped user with unchanged mass-assignment rejection and gives the mobile app a name-edit flow that commits only server-confirmed, same-owner first/last name through the authenticated coordinator and is denied offline by the IP-2.3 guard. Password recovery's email-provider prerequisite is resolved (D-018) and its token/email plumbing is delivered by IP-2.9, leaving the reset endpoints/UI; account deletion and its durable cleanup runner are the next selected slice after the retention/reauthentication decision gate. If that decision is unavailable, IP-2.5 is the next implementable package. The "recovery, edit, delete" audit finding remains open until all three slices and their MC-level proof land.
