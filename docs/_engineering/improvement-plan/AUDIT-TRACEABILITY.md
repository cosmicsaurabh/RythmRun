...
- Current package under review: IP-2.1 replaces plaintext single-token storage with digest-only bounded sessions, standard claims, serializable rotation/replay revocation, active-session access checks, presented-session logout, all-session password revocation, and safe `/users/me`. Local schema/type/unit/HTTP gates pass; the enabled disposable-PostgreSQL CI suite and destructive staging cutover remain pending under MC-2.1/MC-2.2.
- Manual/hosted gates: [MC-0.1 through MC-0.12, MC-1.1 through MC-1.14, and MC-2.1 through MC-2.2](./MANUAL-CHECKS.md), including hosted CI, independent failure probes, required checks, dependency review, security operations, metric sampling/backups, compatibility, GPS/account-exit/database-migration/device proof, bounded-ingest staging, real PostgreSQL/TLS/pool/migration proof, auth-session concurrency/cutover, artifact/deploy ordering, SIGTERM cleanup, ad-device safety, and controlled rollout.
