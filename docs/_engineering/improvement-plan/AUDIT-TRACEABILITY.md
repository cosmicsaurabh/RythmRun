...
- Manual/hosted gates: [MC-0.1 through MC-0.12, MC-1.1 through MC-1.14, and MC-2.1 through MC-2.5](./MANUAL-CHECKS.md), including hosted CI, independent failure probes, required checks, dependency review, security operations, metric sampling/backups, compatibility, GPS/account-exit/database-migration/device proof, bounded-ingest staging, real PostgreSQL/TLS/pool/migration proof, auth-session concurrency/cutover, secure mobile credential migration, Google identity configuration/lifecycle, email-verification provider/delivery/device proof, artifact/deploy ordering, SIGTERM cleanup, ad-device safety, and controlled rollout.
