...
- Repository-delivered; migration application pending: IP-2.5 route privacy is merged to main via PR #165 (`bd78d9a`). `Activity.isPublic` defaults false with a forward migration backfilling existing rows to private; `getActivityById` is owner-only so no cross-user route point, signed image URL, or owner identity is returned (list/update/delete/image routes were already owner-scoped); the friend/comment/like routers are unmounted (`404`) with the backend README/seed corrected. 290 backend Jest tests pass (9 new); no Flutter change was required. The private-by-default migration still needs staging/production application, and the published policy pages that still describe social/public sharing remain to be qualified.
- Repository-delivered (abuse-control slice only); deployed edge configuration pending: IP-2.6 closes the "no rate limits and permissive CORS" finding at the code level on branch `feat/api-abuse-controls`. `app.use(cors())` becomes an exact-match `CORS_ALLOWED_ORIGINS` allowlist that is required and https-only under `NODE_ENV=production` and validated before the listener binds; `TRUST_PROXY_HOPS` drives `trust proxy` and defaults to 0 so a forged `X-Forwarded-For` cannot select a rate-limit key; in-process sliding-window budgets cover login, register, Google exchange, password-reset request and submit, password change, and verification resend, returning a typed `AUTH_RATE_LIMITED` 429 with `Retry-After`, charged on admission and refunded when a failure-counting response is not 4xx, with an added 20-failures-per-address login ceiling closing the credential-spraying gap the account+address key leaves open; the mounted activity-image controller replaces exact-message branching with typed `ActivityImageServiceError` codes; and server-minted request ids plus fixed-field `security_event` lines carry only a truncated subject digest. 452 backend Jest tests (up from 373) and 349 Flutter tests (up from 347) pass with production build and built smoke. **This does not close the other two IP-2.6 rows**: presigned uploads still do not enforce object size for activity images and image confirmation still trusts declared metadata (items 6–8), and the typed-error row is only partly satisfied because the unmounted social controllers still branch on message strings. Deployed proxy depth, production origins, fail-closed boot, live 429 recovery, and the single-replica assumption are gated by MC-2.6.
- Repository-delivered identity extension; operational verification pending: Google Sign-In is merged through `c805f62`. The backend verifies provider tokens against one required audience, requires verified email, keys accounts by provider subject, refuses implicit email linking (on `origin/main`; narrowed by the branch-pending IP-2.9 to allow verified-email auto-linking — see D-018), and issues the existing revocable session; Flutter refuses cleartext exchange and coordinates native identity work with the same auth gate and root navigation. Current local gates pass with 307 executable backend tests (seven PostgreSQL tests skipped), production build/built smoke, 330 Flutter tests, 9 informational analyzer findings and zero warnings/errors, baseline acceptance, formatting, and Android debug APK. This does not close an original audit finding or IP-2.4; MC-2.4 retains migration/provider/device/staging proof.
