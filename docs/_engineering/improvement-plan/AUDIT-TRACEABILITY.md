...
Merged repository implementation evidence is recorded in the applicable phase log. A finding is not considered deployed or production-verified until the applicable migration, staging, deployment, incident, and operational evidence is also present.
...
- Repository-delivered; staging verification pending: IP-1.5 presence-aware serializable PATCH collection replacement, bounded pre-transform activity validation/error output, and a measured route-specific 3 MiB activity parser behind authentication and in-process admission concurrency are committed in `d2c1b95`. That admission guard bounds concurrent activity mutations and is a separate `429` source from the IP-2.6 request budgets; evidence for either must say which one it exercised. The mobile client emits UTC timestamps and records permanent `400`/`413`/`422` sync rejections while leaving auth, admission, server, and network failures retryable. Deployed proxy/resource/PostgreSQL rollback/concurrency and timestamp-compatibility proof remain open under MC-1.8.
...
- Repository-delivered; packaged-device verification pending: IP-1.7 keeps ads disabled/no-op outside an explicitly valid Android production release, validates production identifiers before packaging, and places the one configurable opportunity after durable workout completion. MC-1.14 and IP-5.5 remain open.
...
- Current canonical package (profile and password-recovery slices delivered): IP-2.4's profile-edit slice makes `PUT /profile` return the updated safe `/me`-shaped user with unchanged mass-assignment rejection and gives the mobile app a name-edit flow that commits only server-confirmed, same-owner first/last name through the authenticated coordinator and is denied offline by the IP-2.3 guard. Password recovery is delivered and merged to main via PR #164 (backend `34a14a9` + frontend `6b7dc97`): an anti-enumerating request endpoint, a one-transaction single-use reset that revokes all sessions and refuses Google-only accounts, a deep-link-free backend web form, and a Flutter forgot-password screen, all reusing the IP-2.9 hashed-token store; production exposure is gated by MC-2.5; address-dimension recovery rate limits were IP-2.6 and are now delivered by its abuse-control slice. Account deletion and its durable cleanup runner are the last IP-2.4 slice, selected after the retention/reauthentication decision gate; if that decision is unavailable, IP-2.6 was the next implementable package and its abuse-control slice has now been taken (see the separate IP-2.6 entry); IP-2.6's storage-boundary items 6-8 remain open. On 2026-08-11 (`028d469`) the settings screen's deletion control stopped presenting a type-`DELETE` confirmation dialog that ended in "Account deletion feature coming soon!" and now opens the public deletion-request page instead; that removes a false in-app claim and delivers **no** deletion endpoint, reauthentication, cleanup outbox, runner, or local purge. The "recovery, edit, delete" audit finding remains open until account deletion and all three slices' MC-level proof land.
- Repository-delivered; migration application pending: IP-2.5 route privacy is merged to main via PR #165 (`bd78d9a`). `Activity.isPublic` defaults false with a forward migration backfilling existing rows to private; `getActivityById` is owner-only so no cross-user route point, signed image URL, or owner identity is returned (list/update/delete/image routes were already owner-scoped); the friend/comment/like routers are unmounted (`404`) with the backend README/seed corrected. 290 backend Jest tests pass (9 new); no Flutter change was required. The private-by-default migration still needs staging/production application. The public privacy/terms pages were reconciled with the repository behavior on 2026-07-27; qualified legal/privacy review against the release candidate remains IP-5.6.
- Repository-delivered (abuse-control slice only); deployed edge configuration pending: IP-2.6 closes the "no rate limits and permissive CORS" finding at the code level, merged to main via PRs #167/#169. `app.use(cors())` becomes an exact-match `CORS_ALLOWED_ORIGINS` allowlist that is required and https-only under `NODE_ENV=production` and validated before the listener binds; `TRUST_PROXY_HOPS` drives `trust proxy` and defaults to 0 so a forged `X-Forwarded-For` cannot select a rate-limit key; in-process sliding-window budgets cover login, register, Google exchange, password-reset request and submit, password change, and verification resend, returning a typed `AUTH_RATE_LIMITED` 429 with `Retry-After`, charged on admission and refunded when a failure-counting response is not 4xx, with an added 20-failures-per-address login ceiling closing the credential-spraying gap the account+address key leaves open; the mounted activity-image controller replaces exact-message branching with typed `ActivityImageServiceError` codes; server-minted request IDs plus fixed-field `security_event` lines carry only a truncated subject digest; and Flutter renders rate limits and invalid credentials without leaking exception class text. 452 backend Jest tests (up from 373) and 355 Flutter tests (up from 347) passed at the slice's own snapshot, with production build and built smoke; current main stands at 464 passed / 7 skipped and 359 Flutter tests after later unrelated commits. **This does not close the other two IP-2.6 rows**: presigned uploads still do not enforce object size for activity images and image confirmation still trusts declared metadata (items 6–8), and the typed-error row is only partly satisfied because the unmounted social controllers still branch on message strings. The 2026-07-28 avatar re-hardening makes item 6 cheaper without closing it — `getPresignedPutUrl` can now sign `content-length`, and the activity-image caller still does not pass a size. Deployed proxy depth, production origins, fail-closed boot, live 429 recovery, and the single-replica assumption are gated by MC-2.6.
- Repository-delivered correction; real-storage verification pending: the avatar half of the "unsafe upload/storage boundary" area was re-hardened on main (`76fa16f`, PR #170) after the presigned POST policy behind IP-0.4 turned out to be unsupported by Cloudflare R2 — meaning the byte bound this traceability file previously counted as code-delivered was inert in this deployment. The grant is now a presigned PUT with `content-type` and `content-length` in the signed header set; `R2_PUBLIC_URL` and public avatar delivery are removed in favor of an authenticated `GET /avatar/read-url` short-lived presigned GET; and avatar confirm/cleanup name the avatar bucket explicitly rather than inheriting the activity-image default. **No real-R2 claim is made** — that storage actually rejects an oversize body against a signed `content-length` is MC-0.11, which must be re-run because the client mechanism changed, and MC-0.5 must confirm public delivery is retired rather than merely unused.
- Repository-delivered identity extension; operational verification pending: Google Sign-In and IP-2.9 safe linking are merged to main. The backend verifies provider tokens against one required audience, requires provider-verified email, keys accounts by provider subject, auto-links only a matching password account whose email is already verified, returns `AUTH_EMAIL_UNVERIFIED_CONFLICT` for an unverified collision, and issues the existing revocable session; Flutter refuses cleartext exchange and coordinates native identity work with the same auth gate and root navigation. The historical identity-extension evidence was 307 executable backend tests (seven PostgreSQL tests skipped), 330 Flutter tests, production build/built smoke, analyzer/baseline acceptance, formatting, and Android debug APK; current-tree counts are recorded in the IP-2.6 entry above rather than retroactively attributed to that earlier run. This does not close an original audit finding or IP-2.4; MC-2.4 retains migration/provider/device/staging proof.
- Manual/hosted gates: [MC-0.1 through MC-0.12, MC-1.1 through MC-1.14, and MC-2.1 through MC-2.6](./MANUAL-CHECKS.md), including hosted CI, independent failure probes, required checks, dependency review, security operations, metric sampling/backups, compatibility, GPS/account-exit/database-migration/device proof, bounded-ingest staging, real PostgreSQL/TLS/pool/migration proof, auth-session concurrency/cutover, secure mobile credential migration, Google identity configuration/lifecycle, email-verification provider/delivery/device proof, deployed abuse-control edge configuration, artifact/deploy ordering, SIGTERM cleanup, ad-device safety, and controlled rollout.
...
| No proven operational CI and narrow HTTP-level security coverage | Repository definitions delivered in IP-0.7a/IP-1.6; operational proof and later expansion in IP-5.3 | Successful GitHub Actions run URL, Express regressions, required checks, and intentional-failure probes |
| Historical 159 Flutter analyzer findings; current tree has 9 information findings and no warning/error, but no hosted gate yet | Baseline protection IP-1.6; reduction continued through IP-2; release gate IP-5.3 | Counted fingerprint baseline does not increase; hosted warning/new-info probes fail; remaining information backlog is then reduced |
...
| Conflicting Android Gradle files | Duplicate authority was removed in `a9f2535`; IP-3.4/IP-5.5 retain foreground-service, signing, release, and future Gradle/AGP/Kotlin modernization proof | One authoritative clean foreground-service/release build configuration with supported toolchain versions |
| Google identity provider added outside the original audit sequence | Repository implementation merged; MC-2.4 owns non-rolling migration, OAuth configuration, device/staging lifecycle, branding, and optional-iOS policy | Provider-console/signing matrix, migration rehearsal, first/repeat/conflict/revocation flows, safe outage behavior, and redacted release logs |
