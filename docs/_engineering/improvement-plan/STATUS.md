...
| 2.7 Protect retained routes/photos at rest | Verification | ✓ | Code delivered: logout clears local SQLite data (workouts, points, status changes, images); login triggers full background bootstrap from `GET /api/activities` with pagination, dedup via `hasWorkout`, `history_restored` flag for kill-recovery, and a reactive `SyncHistoryBanner`. **The clearing step deletes unsynced workouts and queued deletes — P0 `SYNC-01` in the [sync audit](../sync/sync-reliability-audit.md).** At-rest encryption (threat model, library/perf, backup, key-loss) remains a future design spike |
...
| 2026-08-13 | IP-2.7 full background bootstrap on login — `downloadAndRestoreWorkouts` fetches paginated server history via `GET /api/activities` (existing backend API, no backend changes), deduplicates via `hasWorkout` (by `clientSyncId` / `remoteActivityId`), persists to SQLite with `ActivitySyncModel.fromJson` mapping. `SyncCoordinator` checks `history_restored` flag (per-user in `SharedPreferences`) to skip on subsequent launches; after an app kill the bootstrap restarts from page 1 (there is no page checkpoint) and `hasWorkout` makes the re-run idempotent. `SyncProgress` enum + `syncProgressProvider` drives a reactive `SyncHistoryBanner` on the home screen. History screen auto-refreshes on restore completion. | Branch `auth-impr` (PR pending / pushed to remote) |
| 2026-08-17 | IP-3.4 design proposal — [Android background tracking and notification controls](../tracking/android-background-tracking-design.md): verified current facts (target 36, manifest, geolocator settings, memory-only workout, no FGS), compared geolocator's foreground mode / `flutter_foreground_task` / an own minimal Kotlin service, and recommends the latter with a cached FlutterEngine, SQLite v7 checkpoints, per-point durable writes, user-scoped recovery, and no `ACCESS_BACKGROUND_LOCATION`. Awaiting maintainer approval; no code changed. | Branch `auth-impr` |
| 2026-08-17 | Push/pull sync reliability audit — static trace of `d0e5b92`; verdict NOT READY. One P0: session teardown (logout, account switch, forced loss incl. 7-day refresh expiry while offline) deletes unsynced workouts and queued deletes. P1: no delta pull/tombstones (no multi-device propagation), restore failure blocks push, no sync visibility, routes > 12,000 points permanently unsyncable, restore path untested. Gap list `SYNC-01`…`SYNC-18` and deferred decisions `D-01`…`D-14` in [`docs/_engineering/sync/sync-reliability-audit.md`](../sync/sync-reliability-audit.md). No code changed. | Branch `auth-impr` |
| 2026-08-17 | GPS tracking audit — static trace of `d0e5b92` from Start → raw position → acceptance → metrics → map → Finish → SQLite. Verdict: short screen-on foreground workouts are correct and safe (IP-1 claims hold in code); screen-off and process-death workouts are not supported (IP-3 gaps, 0 of 5); multi-hour sessions degrade linearly per tick. Findings `F1`…`F14` with severity, evidence, smallest fix and required test, one stale IP-3 audit bullet (the map no longer holds a second GPS subscription), and a phased plan aligned to IP-3.1–3.5 with rollback and device-test requirements in [`docs/_engineering/tracking/gps-tracking-audit.md`](../tracking/gps-tracking-audit.md). No tests, builds, or devices were run. No code changed. | Branch `auth-impr` |
| 2026-08-17 | GPS route-quality and metric-correctness audit — static read of the acceptance policy, timeline, segmenter, provider, elevation/pace/speed helpers, map renderers, sync model, and the pinned geolocator plugin sources, plus one synthetic Python replay of the app's exact arithmetic (no device, no real coordinates). Verdict: units, pause/resume, and the single accepted sequence are correct; GPS *noise* is not yet handled — stationary drift and warm-up fixes count as distance, max speed is a single-interval noise statistic, elevation both over-counts flat noise and under-counts gentle climbs, and the plugin's "missing field → 0.0" contract defeats the accuracy and altitude checks. Findings `RQ-1`…`RQ-17`, threshold recommendations with the device-measurement items called out, a synthetic-only test matrix `T1`…`T20`, and a staged plan that versions new rows (`gps_policy_version`) and never rewrites old ones, in [`docs/_engineering/tracking/gps-route-quality-audit.md`](../tracking/gps-route-quality-audit.md). No tests, builds, or devices were run. No code changed. | Branch `auth-impr` |
| 2026-08-17 | Battery and active-workout durability audit — static read of `d0e5b92` plus pinned plugin/framework sources. Verdict: short screen-on workouts safe; screen-off workouts unsupported and unsafe; multi-hour workouts screen-on only, degrading per second. Battery findings `B-01`…`B-12` (no FGS/wake-lock plumbing; GPS stays on while paused; a 10-s TCP probe to public DNS for the process lifetime; whole-state watches rebuild the map every second and flutter_map drops its projection cache each rebuild; follow-mode leaks two animation status listeners per point; sqflite runs `TRUNCATE`/`FULL` on AOSP), durability findings `D-01`…`D-10` with an 18-row failure-boundary matrix, a recommended architecture that endorses the IP-3.4 design with three departures (no default wake lock, GPS off on pause, lifecycle-gated tick), a PR-level plan, exact tests, drift list, and open decisions in [`docs/_engineering/tracking/battery-durability-audit.md`](../tracking/battery-durability-audit.md). Local build artifacts indicate a debug assemble ran on the bumped toolchain (unrecorded; release unproven). No tests, builds, or devices were run. No code changed. | Branch `auth-impr` |
| 2026-08-18 | Workout-tracking UX and accessibility audit — static read of the Track screen, live provider/state, live map, recovery card, logout/exit dialogs, theme/colours, manifest, existing tests, and the pinned `geolocator_android-5.0.3` permission source. Verdict: honest on the screen-on happy path, not release-honest off it and not accessible on any path. Findings `UX-01`…`UX-27` (lost GPS / no fix / "Approximate" grant leave the timer and *Pause* implying tracking; *Open App Settings* never opens Settings; a stale granted flag loops Start into a generic failure; no "saved" acknowledgement; unconfirmed discard on logout; committed debug `Text`s in the semantics tree; zero `Semantics` in `lib/`, a 24 dp toggle, an unlabelled 40×4 handle, ≈1.8–3.4:1 snackbar contrast in light mode), a journey table, exact user-facing strings `S0`…`S18`, a recovery decision tree, notification-label/two-tap-Finish recommendations against the IP-3.4 design, test requirements, and a five-stage plan of which stages 0–2 need no schema, in [`docs/_engineering/tracking/workout-tracking-ux-accessibility-audit.md`](../tracking/workout-tracking-ux-accessibility-audit.md). Contrast values computed, not measured. No tests, builds, or devices were run. No code changed. | Branch `auth-impr` |
| 2026-08-18 | Map rendering and tile-provider reliability audit — static read of `LiveMapFeed`, `WorkoutHistoryMapViewer`, the attribution widget, map helpers, `pubspec`, and the pinned `flutter_map 8.3.1` / `http 1.6.0` / Flutter 3.44.1 sources; OSMF Tile Usage Policy and Attribution Guidelines fetched 2026-08-18 and quoted. Verdict: **Amber** — tracking and SQLite save never depend on tiles; URL/HTTPS/attribution/HTTP caching/no-prefetch comply with the current policy; gaps are the library-formatted User-Agent, the hard-coded endpoint in two files, tile URLs `debugPrint`ed to logcat in release on every failed tile, passive degradation (blank tiles, no indicator, no retry on reconnect, no live route-only mode), a launch-time GPS fix/permission prompt from the map, an unscoped device-wide tile cache, and two per-rebuild/per-point growth defects (leaked animation status listeners replaying every past destination; a new `HttpClient` per rebuild). Findings `M-01`…`M-16`, facts `F-01`…`F-40` with `file:line`, a compliance table, degraded-mode UX flow, provider strategy (personal / small release / offline — offline is not permitted on `tile.openstreetmap.org`), tests `T-1`…`T-9`, and a staged plan with no paid infrastructure unless a stated trigger fires, in [`docs/_engineering/tracking/map-tile-reliability-audit.md`](../tracking/map-tile-reliability-audit.md). No tests, builds, devices, or requests to the tile server. No code changed. | Branch `auth-impr` |
