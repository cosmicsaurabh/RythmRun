...
| MC-2.4 | Google identity migration, provider configuration, and device lifecycle | Database, deployment, mobile, security, and QA owners | Pending | Reviewed non-rolling migration/artifact record plus OAuth-console/signing matrix and synthetic Android staging run covering first/repeat/cancel/conflict/switch/refresh/logout/outage flows and redacted logs | — | — | Never record provider/RythmRun tokens, OAuth client secrets, raw profile bodies, signing material, exact routes, or raw logs. OAuth client IDs are identifiers but should still be referenced through the approved configuration record. iOS is optional under D-008; if released, add callback-scheme, physical-device, Apple-policy, and review evidence. |
| MC-2.5 | Email provider, delivery, and device lifecycle (IP-2.9 verification + IP-2.4 password reset) | Deployment, backend, mobile, security, and QA owners | Pending | Applied additive `emailVerified`/`VerificationToken` migration (and the `PASSWORD_RESET` enum migration `20260721000000`) on the release DB before the linking code serves traffic; real Brevo/SMTP configuration with sender-domain SPF/DKIM/DMARC authenticated; staging run delivering a live verification email, rendering the verify page (success/already/expired), consuming a single-use token idempotently, and exercising the throttled resend; on-device banner shown only to unverified password accounts with resend + "I've confirmed" refresh clearing it; a verified-email Google sign-in auto-links while an unverified one returns `AUTH_EMAIL_UNVERIFIED_CONFLICT`; **and, for password reset, delivering a live reset email, rendering the reset form from the emailed link, submitting it to consume the single-use 30-minute token once, proving all sessions are revoked, confirming a Google-only account request sends nothing, and driving the same flow from the Flutter forgot-password screen**; redacted logs confirming no token/digest/recipient is written for either flow | — | — | Use synthetic accounts and a capture-only inbox or disposable mailbox in staging. Store no raw token, digest, recipient address, SMTP key, or raw log in Git. Verifies IP-2.9 email verification and the IP-2.4 password-recovery production exposure; local backend/Flutter tests alone cannot mark it verified. |
| MC-2.6 | Deployed edge configuration for abuse controls (IP-2.6) | Deployment, backend, and QA owners | Pending | Recorded `TRUST_PROXY_HOPS` value with the reasoning that ties it to the actual hosting proxy depth, plus a staging probe proving a client-supplied `X-Forwarded-For` cannot change the address the limiter charges; recorded production `CORS_ALLOWED_ORIGINS` value with a staging probe showing an allowlisted origin receives an exact `Access-Control-Allow-Origin` and a non-allowlisted one receives none, with no wildcard and no `Access-Control-Allow-Credentials`; a deployment smoke proving a production process with `CORS_ALLOWED_ORIGINS` absent exits non-zero before listening; a staging run driving one endpoint past its budget and confirming a stable `429` with `Retry-After` that recovers after the window; confirmation the deployment still runs exactly one replica, or a recorded decision to move the counters to a shared store before scaling | — | — | Record only the configured values and probe results; store no raw logs, client addresses, or account identifiers. The limiter is in-process, so restart clears every counter and a second replica multiplies each limit — the replica count is part of what this check verifies. Distinct from the IP-1.5 admission-concurrency `429` exercised by MC-1.8 step 8; both sources must be told apart when reading evidence. Local backend tests prove the code honours the configured hop count, not that the configured value matches the real proxy. |
...
1. Push the reviewed current auth/session tree—at or after merged Google identity tree `c805f62`—through the ordinary pull-request workflow. Confirm `Backend security` resolves that SHA, provisions its disposable `rythmrun_ci` PostgreSQL service, and applies the complete migration chain before tests.
2. Inspect the Jest summary and confirm all seven current `auth-session.postgres.test.ts` cases execute; a skipped database suite is a failed gate even when the job is otherwise green.
...
### MC-2.4 — Verify Google identity migration and lifecycle

1. Start from the reviewed merged commit/artifact and an isolated staging environment containing synthetic password accounts. In the restricted configuration record, identify the Google Cloud project, consent-screen mode, one web/server OAuth client, Android package name, and every approved debug/internal/release signing-certificate fingerprint. The exact web client ID configured as backend `GOOGLE_SERVER_CLIENT_ID` must be the audience supplied to Flutter; no OAuth client secret belongs in the mobile app.
2. On a representative upgrade copy, run the migration preflight that rejects blank or case/whitespace-colliding usernames. Drain and stop every old backend instance before applying `20260715000000_add_google_auth`; keep traffic drained until the matching Google-aware artifact is promoted. Prove old and new artifacts are never live together across the nullable-password/authentication-method schema boundary, and rehearse a roll-forward response without recreating invalid legacy values.
3. Confirm the hosted `Backend security` run applies the complete migration chain and executes the seven-test PostgreSQL suite rather than skipping it. In staging, verify the new canonical/subject constraints exist and a Google-only account cannot exist without a provider subject. Record only safe aggregate/pass references, never rows or tokens.
4. On supported Android release-signed device classes, exercise chooser cancellation, first Google sign-in, repeat sign-in, process restart, access expiry/refresh, logout, deliberate account switching, and backend-forced session revocation. Success must use the same RythmRun secure credential envelope, authenticated root, offline policy, and teardown rules as password login; cancellation must create no backend/local session.
5. Use synthetic identities to prove the account-collision policy: an existing password account with the same email receives a safe conflict and is not linked; the same verified Google subject returns the same account; concurrent first exchanges create at most one provider account; a changed provider email does not replace subject identity. Confirm Google-only Settings hides password change and profile editing still follows the safe same-owner contract.
6. Exercise invalid, expired, wrong-audience, unverified-email, and malformed tokens plus a controlled provider-certificate/network outage. Invalid identity returns the generic typed rejection, outage returns only the safe retryable category, no exchange is automatically replayed, and no client-supplied subject/email/name is trusted as proof. All exchange traffic must use HTTPS.
7. Inspect the final release UI against Google's then-current branding requirements; the temporary plain `G` placeholder is not release evidence. Review sanitized release logs/exported diagnostics for unique synthetic sentinels: no Google ID token, RythmRun token, authorization header, OAuth secret, raw profile body, signing material, secure-store value, exact route, or local path may appear.
8. Under D-008, Android is the required platform. If iOS enters release scope, additionally prove the iOS client ID and reversed callback scheme in the signed artifact, physical-device sign-in/logout/account-switch behavior, secure storage, and the applicable App Store requirement for an equivalent Sign in with Apple option before submission.

Pass condition: the non-rolling database/artifact cutover, exact audience/signing configuration, Android provider and first-party session lifecycle, collision/concurrency/outage behavior, approved branding, and redacted logs have dated owner and independent reviewer evidence. Repository tests, an OAuth-console screenshot alone, or a debug APK cannot mark MC-2.4 verified.
...
1. Use an isolated test account and a reviewed release build on representative supported Android devices; record only build/device references in Git. iOS is optional until IP-5 changes the Android-only scope in D-008.
...
Pass condition: required Android evidence shows one accepted route across UI/local/sync, deterministic pause timing, no resume bridge, and no exact-route logging. If iOS is later declared supported, repeat the same gate there before making an iOS readiness claim.
