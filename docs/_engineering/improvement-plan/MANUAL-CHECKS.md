...
| MC-2.3 | Android secure-credential migration and session lifecycle | Mobile, security, backend, and QA owners | Pending | Reviewed release-build/device matrix covering in-place preference migration and interruption recovery, encrypted-at-rest inspection, backup/restore exclusion, sanitized logs, concurrent expiry refresh, offline/invalid separation, login/register/password/logout roots, and rollback | — | — | Use synthetic accounts and sentinel credentials only in isolated staging. Store no token, device file, screenshot containing a token, profile body, route, or raw log in Git. Android is the promised platform under D-008; iOS evidence is optional until IP-5 changes scope. |
