...
| MC-2.5 | Email-verification provider, delivery, and device lifecycle (IP-2.9) | Deployment, backend, mobile, security, and QA owners | Pending | Applied additive `emailVerified`/`VerificationToken` migration on the release DB before the linking code serves traffic; real Brevo/SMTP configuration with sender-domain SPF/DKIM/DMARC authenticated; staging run delivering a live verification email, rendering the verify page (success/already/expired), consuming a single-use token idempotently, and exercising the throttled resend; on-device banner shown only to unverified password accounts with resend + "I've confirmed" refresh clearing it; a verified-email Google sign-in auto-links while an unverified one returns `AUTH_EMAIL_UNVERIFIED_CONFLICT`; redacted logs confirming no token/digest/recipient is written | — | — | Use synthetic accounts and a capture-only inbox or disposable mailbox in staging. Store no raw token, digest, recipient address, SMTP key, or raw log in Git. Verifies IP-2.9; local backend/Flutter tests alone cannot mark it verified. |
