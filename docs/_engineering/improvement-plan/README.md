...
> Program status: IP-0 remains the release blocker and its operational evidence is still open. The maintainer has explicitly selected repository-only IP-2 work in parallel. IP-2.9 email verification (PR #164) and IP-2.5 route privacy (PR #165) are now **merged to main**, alongside the earlier Google identity extension. Within IP-2.4, profile edit and password recovery are delivered (merged via PR #164); **account deletion is the only remaining IP-2.4 slice**, pending its retention/reauthentication decision gate. Password recovery reuses IP-2.9's hashed token/email plumbing; its production exposure is gated by MC-2.5 and address-dimension rate limits remain IP-2.6. **IP-2.6 (API abuse controls / typed errors) is the next implementable package.** All of this remains repository-delivered with local tests only; MC-2.4/MC-2.5 still owe provider, migration, device, and staging proof, and none of it authorizes migration execution, deployment, or production enablement.
...
1. **IP-2.6 abuse and upload controls:** deploy an explicit CORS/proxy contract, rate-limit auth/account routes (including the Google exchange, verify/resend, and password-reset request endpoints), finish typed error mapping, and enforce actual storage size/type/integrity plus abandoned-upload cleanup. This is the next implementable package now that IP-2.5 is merged.
