...
> Program status: IP-0 remains the release blocker and its operational evidence is still open. The maintainer has explicitly selected repository-only IP-2 work in parallel. IP-2.9 email verification (PR #164) and IP-2.5 route privacy (PR #165) are **merged to main**, alongside the earlier Google identity extension. Within IP-2.4, profile edit and password recovery are delivered; **account deletion is the only remaining IP-2.4 slice**, pending its retention/reauthentication decision gate. The IP-2.6 abuse-control slice is repository-delivered on `feat/api-abuse-controls`: CORS/proxy configuration, authentication and recovery request budgets, typed mounted-image errors, request IDs, security events, and matching Flutter auth-error messages are implemented. IP-2.6 remains `In progress` because its storage-boundary items 6–8 are open. MC-2.4 through MC-2.6 still owe provider, migration, device, staging, and deployed-edge proof, and none of this authorizes migration execution, deployment, or production enablement.
...
- **Repository-delivered abuse controls; deployed verification pending:** IP-2.6 items 1, 2, 4, 5, and 9 plus the proxy-aware portion of item 3 are delivered on `feat/api-abuse-controls`. The branch has an exact production CORS allowlist, explicit proxy-hop trust, in-process authentication/recovery budgets, stable typed `429` responses, typed errors in the mounted activity-image controller, server-minted request IDs, privacy-minimized security events, and readable Flutter handling for rate limits and invalid credentials. Backend 452-test and Flutter 355-test suites pass locally. Storage-boundary items 6–8 and MC-2.6 remain open.
...
The audit baseline on 2026-07-10 was 25 backend tests passing, TypeScript passing, 15 Flutter tests passing, and 159 Flutter analyzer findings (6 warnings and 153 informational findings). IP-1.6 reduced that to 20 informational findings and zero warnings/errors; IP-2.2 reduced it to 10, and the current tree is locally at 9 informational findings and zero warnings/errors. The current suites contain 355 Flutter tests and 459 backend tests, of which 452 execute locally while seven real-PostgreSQL tests remain intentionally gated. The committed multiset baseline prevents increases but does not make the remaining findings acceptable forever.
