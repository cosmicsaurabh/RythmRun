...
> Program status: IP-0 remains the release blocker and its operational evidence is still open. The maintainer has explicitly selected repository-only IP-2 work in parallel. IP-2.9 email verification (PR #164), IP-2.5 route privacy (PR #165), and the IP-2.6 abuse-control slice (PRs #167/#169) are all **merged to main**, alongside the earlier Google identity extension. Within IP-2.4, profile edit and password recovery are delivered; **account deletion is the only remaining IP-2.4 slice**, pending its retention/reauthentication decision gate — the settings screen now links to the public deletion-request page, which removes a "coming soon" placeholder rather than delivering any part of deletion. IP-2.6 remains `In progress` because its storage-boundary items 6–8 are open. Separately, IP-0.4 was re-hardened on 2026-07-28 (PR #170): the avatar upload grant was a presigned POST policy that Cloudflare R2 does not support, so its byte bound never actually bound; it is now a presigned PUT with a signed `content-length`, and public avatar delivery has been replaced by an authenticated short-lived presigned GET. MC-0.11 must be re-run against that changed mechanism. MC-2.4 through MC-2.6 still owe provider, migration, device, staging, and deployed-edge proof, and none of this authorizes migration execution, deployment, or production enablement.
...
- **Current canonical package; delivered in slices:** IP-2.4 profile edit is repository-delivered: `PUT /profile` returns the updated safe `/me`-shaped user with unchanged mass-assignment rejection, and the mobile app gains the name-edit flow that commits only server-confirmed, same-owner first/last name through the authenticated coordinator, denied offline by the IP-2.3 guard. Password recovery is delivered too (merged to main via PR #164, backend `34a14a9` + frontend `6b7dc97`): an anti-enumerating request endpoint, a one-transaction single-use reset that revokes all sessions and refuses Google-only accounts, a deep-link-free backend web form, and a Flutter forgot-password screen — reusing IP-2.9's hashed-token store. Its account/address request budget is delivered by the IP-2.6 abuse-control slice; production exposure remains gated by MC-2.5. IP-2.5 route privacy is also merged (PR #165). Account deletion with a durable object-cleanup outbox and runner is the last IP-2.4 slice, selected after its retention/reauthentication decision gate. The recovery/edit/delete audit finding remains open until account deletion and all three slices' MC-level proof land.
- **Repository-delivered abuse controls; deployed verification pending:** IP-2.6 items 1, 2, 4, 5, and 9 plus the proxy-aware portion of item 3 are merged to main via PRs #167/#169. Main has an exact production CORS allowlist, explicit proxy-hop trust, in-process authentication/recovery budgets, stable typed `429` responses, typed errors in the mounted activity-image controller, server-minted request IDs, privacy-minimized security events, and readable Flutter handling for rate limits and invalid credentials. Storage-boundary items 6–8 and MC-2.6 remain open.
- **Repository-delivered avatar re-hardening; real-storage verification pending:** IP-0.4's upload grant is corrected on main (`76fa16f`, PR #170). The presigned POST policy it previously relied on is unsupported by Cloudflare R2, so the `content-length-range` this program recorded as delivered never enforced anything; the grant is now a presigned PUT whose signed header set includes both `content-type` and `content-length`. `R2_PUBLIC_URL` is retired and an authenticated `GET /avatar/read-url` issues a short-lived presigned GET, so neither bucket needs public access. Avatar confirm and cleanup now name the avatar bucket explicitly instead of inheriting the activity-image default. MC-0.11 is void for the old mechanism and must be re-run; MC-0.5 must confirm public delivery is retired, not merely unused.
- **Repository state; no package advanced:** five later `main` commits are reconciled into the plan as of 2026-08-11 — the avatar re-hardening above, release fixes (OpenStreetMap attribution on both map screens, the deletion-request link, version `1.2.0+21`), an Android toolchain bump (Gradle 8.14 / AGP 8.11.1 / Kotlin 2.2.20, nominally clearing IP-3.4's stated prerequisite but with no build or device proof), an `APP_ENV` dart-define that selects the API base URL and timeout only, and a `dart format` reflow. Current main: backend 464 passed / 7 skipped / 471 total with a clean typecheck; Flutter 359 tests, 9 analyzer issues, zero warnings and errors.
- **Repository-delivered identity extension; operational verification pending:** Google Sign-In and IP-2.9 safe linking are merged to main. The backend verifies Google ID tokens against the configured web-client audience, requires provider-verified email, keys identity by stable provider subject, auto-links only a matching password account whose email is already verified, returns `AUTH_EMAIL_UNVERIFIED_CONFLICT` otherwise, and issues the existing revocable RythmRun session. Flutter uses the Google Sign-In 7.2 API, refuses cleartext exchange, serializes chooser/sign-out work with account teardown, and normalizes success through the same session root as password login. The historical merge evidence remains in the IP-2 phase log; MC-2.4 owns the non-rolling migration, OAuth-console/signing, provider, device, staging, branding, and optional-iOS proof.
- **Manual/hosted gates:** use [MANUAL-CHECKS.md](./MANUAL-CHECKS.md) for hosted CI, intentional-failure, branch-protection, dependency-audit, deployment, incident, infrastructure, staging, and controlled-reopen evidence. MC-1.9 through MC-1.11 keep Flutter/required-check evidence separate from repository delivery; MC-1.12 through MC-1.14 own database/deployment/ad-device proof; MC-2.1 through MC-2.6 own the hosted auth transaction, destructive server cutovers, mobile secure-storage lifecycle, Google identity-provider/device, email-verification provider/delivery/device, and deployed abuse-control edge gates.
...
1. **IP-2.6 storage boundary (items 6–8):** replace the activity-image upload grant with enforceable size/type constraints, verify actual object metadata and integrity, add per-user object/count/byte quotas, expire abandoned uploads, and add presign/storage volume alarms. Coordinate the changed upload contract with Flutter and real-storage proof.
2. **IP-2.4 account deletion:** decide retention and password-versus-fresh-Google reauthentication, then add transactional session revocation, durable object-cleanup outbox/runner, remote deletion state, and complete local database/file/key purge. The last undelivered IP-2.4 slice (password recovery and IP-2.5 route privacy are merged to main).
3. **IP-2.7 retained-data protection:** complete the threat-model/library/performance design gate, then migrate SQLite routes and activity photos to approved user-scoped at-rest protection with backup/key-loss recovery.
4. **IP-3.1 durable checkpoint engine:** begin schema/engine development and dev/test checkpoint persistence behind a disabled-by-default feature flag in parallel with later account work. Do not enable persistence for user data or release the feature until the IP-2.7 design gate approves its at-rest format, key handling, backup behavior, and recovery limits.
...
The audit baseline on 2026-07-10 was 25 backend tests passing, TypeScript passing, 15 Flutter tests passing, and 159 Flutter analyzer findings (6 warnings and 153 informational findings). IP-1.6 reduced that to 20 informational findings and zero warnings/errors; IP-2.2 reduced it to 10, and the current tree is locally at 9 informational findings and zero warnings/errors. As of 2026-08-11 on `main`, the suites contain 359 Flutter tests and 471 backend tests, of which 464 execute locally while seven real-PostgreSQL tests remain intentionally gated. The committed multiset baseline prevents increases but does not make the remaining findings acceptable forever — and it is currently checkable only on the pinned CI toolchain, since the baseline is stamped Flutter 3.44.1 / Dart 3.12.1 and the development machine has moved to 3.44.9 / 3.12.2.
