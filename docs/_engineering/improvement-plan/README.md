...
> Program status: IP-0 remains the release blocker and its operational evidence is still open. The maintainer has explicitly selected repository-only IP-2 work in parallel. IP-2.4 remains the current canonical package: profile edit is delivered, account deletion is the next selected slice after its retention/reauthentication decision gate, and password recovery's email-provider prerequisite is now resolved (D-018) with its reusable token/email plumbing delivered by IP-2.9, leaving the reset endpoints/UI to build. If the deletion decision is unavailable, IP-2.5 is the next implementable package. A separately selected Google identity extension is merged, and a further IP-2.9 email-verification/safe-linking extension is repository-delivered on branch `feat/email-verification` (PR pending); both still require provider, migration, device, and staging proof; none of this authorizes migration execution, deployment, or production enablement.
...
- **Current canonical package; delivered in slices:** IP-2.4 profile edit is repository-delivered: `PUT /profile` returns the updated safe `/me`-shaped user with unchanged mass-assignment rejection, and the mobile app gains the name-edit flow that commits only server-confirmed, same-owner first/last name through the authenticated coordinator, denied offline by the IP-2.3 guard. Password recovery's email-provider prerequisite is now resolved (D-018) and its token/email plumbing is delivered by IP-2.9, leaving the reset endpoints/UI to build; account deletion with a durable object-cleanup outbox and runner is the next selected slice after its retention/reauthentication decision gate. If that decision is unavailable, proceed to IP-2.5 without claiming IP-2.4 complete. The recovery/edit/delete audit finding remains open.
...
1. **IP-2.4 account deletion:** decide retention and password-versus-fresh-Google reauthentication, then add transactional session revocation, durable object-cleanup outbox/runner, remote deletion state, and complete local database/file/key purge. Password recovery's email-provider prerequisite is now resolved (D-018) and its token/email primitives are delivered by IP-2.9 — the remaining recovery work is the reset endpoints/UI, which can proceed in parallel.
