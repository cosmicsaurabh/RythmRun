...
> Program status: IP-0 remains the release blocker and its operational evidence is still open. The maintainer has explicitly selected repository-only IP-2 work in parallel and the current package is IP-2.4, delivered in maintainer-selected slices starting with profile edit; this does not authorize migration execution, deployment, or production enablement.
...
- **Current repository package; delivered in slices:** IP-2.4 profile edit is repository-delivered: `PUT /profile` returns the updated safe `/me`-shaped user with unchanged mass-assignment rejection, and the mobile app gains the name-edit flow that commits only server-confirmed, same-owner first/last name through the authenticated coordinator, denied offline by the IP-2.3 guard. Backend 281-executable-test Jest suite, production build, and built smoke, plus the 302-test Flutter suite, analyzer/baseline, formatting, and Android debug APK pass locally. Password recovery remains blocked on the email-provider decision; account deletion (durable object-cleanup outbox and runner) is the next slice; neither is claimed.
- **Manual/hosted gates:** use [MANUAL-CHECKS.md](./MANUAL-CHECKS.md) for hosted CI, intentional-failure, branch-protection, dependency-audit, deployment, incident, infrastructure, staging, and controlled-reopen evidence. MC-1.9 through MC-1.11 keep Flutter/required-check evidence separate from repository delivery; MC-1.12 through MC-1.14 own database/deployment/ad-device proof; MC-2.1 through MC-2.3 own the hosted auth transaction, destructive server cutover, and mobile secure-storage/device lifecycle gates.
...
The audit baseline on 2026-07-10 was 25 backend tests passing, TypeScript passing, 15 Flutter tests passing, and 159 Flutter analyzer findings (6 warnings and 153 informational findings). IP-1.6 reduced that to 20 informational findings and zero warnings/errors; IP-2.2 reduced it to 10 informational findings and zero warnings/errors, and IP-2.3/IP-2.4 hold at 10 while growing the Flutter suite to 302 tests and the backend suite to 287 (281 executable locally). The committed multiset baseline prevents increases but does not make the remaining findings acceptable forever.
