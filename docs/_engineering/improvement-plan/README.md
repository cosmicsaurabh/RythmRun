...
> Program status: IP-0 remains the release blocker and its operational evidence is still open. The maintainer has explicitly selected repository-only IP-2 work in parallel. IP-2.4 remains the current canonical package: profile edit is delivered, account deletion is the next selected slice after its retention/reauthentication decision gate, and password recovery is blocked on the email-provider decision. If the deletion decision is unavailable, IP-2.5 is the next implementable package. A separately selected Google identity extension is merged but still requires provider, migration, device, and staging proof; none of this authorizes migration execution, deployment, or production enablement.
...
| D-017 | Google authentication exchanges only a provider-verified ID token at the backend, keys accounts by Google's stable subject, never implicitly links an existing password account by email, and issues the same bounded RythmRun session as password login. | Email alone is not a safe account-linking proof. One first-party session contract preserves revocation, secure storage, offline policy, and account-scope teardown while a separate provider/device gate owns OAuth configuration and rollout. |
