...
This merged checkpoint is repository evidence, not production verification. It is not a claim that the deployed service is safe — but note that merging **does** deploy it (see the deployment reality note in [STATUS.md](./STATUS.md)), so "merged" and "running in production" are the same event here. The additive database migration must precede the backend, older clients require a coordinated or forced update, the independent bucket lifecycle rule still needs infrastructure evidence, and IP-0.1/IP-0.6 plus staging and production checks remain open.
...
| 2026-07-10 | IP-0.2–IP-0.5 | `e33f314`, merged through `54a5b26` into `origin/main` | Merged, and live via Render auto-deploy (recorded 2026-08-11; the "not deployed" wording here was wrong) | Repository delivery is confirmed. Migration, staging, production, incident-response, credential-rotation, mobile-rollout, and infrastructure evidence remain open. |
