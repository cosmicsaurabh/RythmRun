    ...
    const result = await this.withSerializableTransaction<RotationResult>(
      async (transaction) => {
        ...
        if (record.usedAt !== null) {
          // A used refresh token was re-presented. Under single-use rotation
          // this is reuse — a stolen token being replayed, or a client retrying
          // after it already rotated (e.g. a lost response). The two are
          // indistinguishable here, so we fail closed: kill the whole family so
          // a stolen token cannot rotate, and the client re-authenticates.
          if (
